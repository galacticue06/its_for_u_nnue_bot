...
##    def search_with_ponder(self, board, wtime, btime, winc, binc, ponder, time_limiter = 1000):
...
##    def search_with_ponder(self, board, wtime, btime, winc, binc, ponder, time_limiter = 0.47):
##        cmds = self.go_commands
##        movetime = cmds.get("movetime")
##        if movetime is not None:
##            movetime = float(movetime) / 1000
...
##        return self.search(board, time_limit, ponder)
    ...
    def search_with_ponder(self, board, wtime, btime, winc, binc, ponder, time_limiter = 0.47):
        ...
        if movetime is not None:
            movetime = float(movetime) / 1000
        time_limit = chess.engine.Limit(time=time_limiter)
        return self.search(board, time_limit, ponder)
